...
void load_spv_from_binary_file(const std::string& file_path, std::vector<uint32_t>& spv_code) {
  ...
  #ifndef NDEBUG
  if (!file) DEBUG_LOG("failed to open: %s", file_path.c_str());
  #endif
  ...
  fread(reinterpret_cast<char*>(spv_code.data()), file_size, 1, file);
  fclose(file);
}
...
void create_vk_xr_swapchain_context(const uint32_t image_count, const XrSwapchainCreateInfo& swapchain_create_info, VkXrSwapchainContext* const swapchain_context) {
  ...
  // create depth buffer
  {
    ...
    depth_image_info.usage         = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
    ...
    vk_allocate_memory(memory_requirements, &swapchain_context->depth_buffer_memory, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
  ...
  // create color buffer
  {
    ...
    color_buffer_info.usage         = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
    ...
    vk_allocate_memory(memory_requirements, &swapchain_context->color_buffer_memory, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
...
bool check_vk_result(VkResult vk_result) {
  if (vk_result == VK_SUCCESS) {
    ...
    DEBUG_LOG("%s\n", vk_result_to_str(vk_result).data());
    return false;
  }
}
...
VkVertexBuffer* create_vk_vertex_buffer(const size_t max_indices_count, const size_t max_vertices_count, VkVertexBuffer* const vertex_buffer) {
  ...
  VkFlags memory_type_flags              = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
  ...
  vkMapMemory(vulkan_logical_device, vertex_buffer->memory, 0, VK_WHOLE_SIZE, 0, &vertex_buffer->mapped_memory);
...
void destroy_vk_vertex_buffer(VkVertexBuffer* const vertex_buffer) {
  vkUnmapMemory(vulkan_logical_device, vertex_buffer->memory);
...
VkInstanceBuffer* create_vk_instance_buffer(const size_t max_instance_count, VkInstanceBuffer* const instance_buffer) {
  ...
  VkFlags memory_type_flags                = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
  VkBufferUsageFlags instance_buffer_usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
  ...
  vkMapMemory(vulkan_logical_device, instance_buffer->memory, 0, VK_WHOLE_SIZE, 0, &instance_buffer->mapped_memory);

  return instance_buffer;
}

void destroy_vk_instance_buffer(VkInstanceBuffer* const instance_buffer) {
  vkUnmapMemory(vulkan_logical_device, instance_buffer->memory);
...
void vulkan_end_then_submit_temporary_command_buffer(VkCommandBuffer tmp_command_buffer) {
  ...
  VkFence temporary_command_buffer_submit_fence;
  VkFenceCreateInfo fence_info{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
...
bool init_graphics() {
  ...
  /* NOTES
   * extension_str is a single space (ASCII 0x20) delimited string of extension names (it is null-terminated)
   * this function modifies the extension_str
   * the pointers in extensions will point to the modified extension_str
   */
  auto build_extensions_vector = [](char* extension_str, std::vector<const char*>& extensions) {
    if (!extension_str) return;
    const size_t extension_str_length = strlen(extension_str);
    if ( !(extension_str_length > 0) ) return;
    extensions.push_back(&extension_str[0]);

    for (size_t i=1; i < extension_str_length; ++i) {
      if (extension_str[i-1] == ' ') {
        extension_str[i-1] = '\0';
        extensions.push_back(&extension_str[i]);
      }
    }
  };

  {
    ...
    /*  NOTE: xrGetVulkanInstanceExtensionsKHR doesn't seem to work with XR_KHR_vulkan_enable2
    PFN_xrGetVulkanInstanceExtensionsKHR xrGetVulkanInstanceExtensionsKHR = nullptr;
    XrResult tmp_result1 = xrGetInstanceProcAddr(platform_xr_instance, "xrGetVulkanInstanceExtensionsKHR", (PFN_xrVoidFunction *)(&xrGetVulkanInstanceExtensionsKHR));
    DEBUG_LOG("%s\n", xr_result_to_str(tmp_result1).c_str());
    uint32_t names_capacity = 0;
    XrResult tmp_result2 = xrGetVulkanInstanceExtensionsKHR(platform_xr_instance, platform_xr_system_id, 0, &names_capacity, nullptr);
    std::vector<char> names_str(names_capacity);
    xrGetVulkanInstanceExtensionsKHR(platform_xr_instance, platform_xr_system_id, names_capacity, &names_capacity, names_str.data());

    build_extensions_vector(names_str.data(), instance_extensions);
    */
    ...
    instance_extensions.push_back("VK_EXT_debug_report"); // needed for VK_EXT_debug_marker device extension (which is only needed to remove a warning log)
    #endif
...

  {
    ...
    /*  NOTE: xrGetVulkanDeviceExtensionsKHR doesn't seem to work with XR_KHR_vulkan_enable2
    PFN_xrGetVulkanDeviceExtensionsKHR xrGetVulkanDeviceExtensionsKHR = nullptr;
    xrGetInstanceProcAddr(platform_xr_instance, "xrGetVulkanDeviceExtensionsKHR", (PFN_xrVoidFunction *)(&xrGetVulkanDeviceExtensionsKHR));
    uint32_t buffer_capacity;
    xrGetVulkanDeviceExtensionsKHR(platform_xr_instance, platform_xr_system_id, 0, &buffer_capacity, nullptr);
    std::vector<char> buffer(buffer_capacity);
    xrGetVulkanDeviceExtensionsKHR(platform_xr_instance, platform_xr_system_id, buffer_capacity, &buffer_capacity, buffer.data());

    build_extensions_vector(buffer.data(), device_extensions);
    */
    ...
    #ifndef NDEBUG
    device_extensions.push_back("VK_EXT_debug_marker"); // removes warning log
    #endif
    ...
    assert(vulkan_physical_device_properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU);
...

  {
    ...
    current_vk_result = vkCreateSampler(vulkan_logical_device, &sampler_info, nullptr, &vulkan_texture_sampler);
...

  {
    ...
    current_vk_result = vkCreateDescriptorSetLayout(vulkan_logical_device, &descriptor_set_layout_info, nullptr, &vulkan_descriptor_set_layout);
    ...
    current_vk_result = vkCreateDescriptorPool(vulkan_logical_device, &descriptor_pool_info, nullptr, &vulkan_descriptor_pool);
    ...
    current_vk_result = vkAllocateDescriptorSets(vulkan_logical_device, &descriptor_set_allocate_info, vulkan_descriptor_sets);
  }
...
void vulkan_render_xr_views(const std::vector<XrCompositionLayerProjectionView>& xr_views, const uint32_t swapchain_image_index) {
  ...
  VkResult fence_wait_result = vkWaitForFences(vulkan_logical_device, 1, &vulkan_xr_swapchain_context->frame_in_flight_fences[current_frame], VK_TRUE, UINT64_MAX);
  while (fence_wait_result == VK_TIMEOUT) {
    DEBUG_LOG("VK_TIMEOUT for vkWaitForFences, frame_in_flight_fences frame index: %d\n", current_frame);
    vkDeviceWaitIdle(vulkan_logical_device);
    fence_wait_result = vkWaitForFences(vulkan_logical_device, 1, &vulkan_xr_swapchain_context->frame_in_flight_fences[current_frame], VK_TRUE, UINT64_MAX);
  }
