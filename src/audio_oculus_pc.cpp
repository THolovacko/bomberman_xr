...
extern std::string get_audio_output_device_id();
...
bool init_audio() {
  {
    ...
    const std::string target_device_id = get_audio_output_device_id();
    ma_uint32 device_index;
    for (device_index=0; device_index < playback_count; ++device_index) {
      std::string current_device_id_wasapi = std::string((LPCTSTR)CString(playback_info[device_index].id.wasapi));
      if ( target_device_id.compare(current_device_id_wasapi) == 0 ) break;
    }

    DEBUG_LOG("Selected audio device: %s\n", playback_info[device_index].name);

    ma_device_config config = ma_device_config_init(ma_device_type_playback);
    config.playback.pDeviceID = &playback_info[device_index].id;
